...
class TgSender(Sender):
    ...
    @staticmethod
    async def _search_linked_message(history: tp.List[tp.Dict[str, str]], linked_node_id: str) -> str:
    ...
    @staticmethod
    async def get_keyboard(event: OutEvent) -> None | aiogram.types.InlineKeyboardMarkup:
        keyboard = None
    ...
    async def send(self, event: OutEvent, history: tp.List[tp.Dict[str, str]]) -> str | None:
        ...
        if event.node_to_edit:
            ...
            message_id_to_edit = await self._search_linked_message(history=history, linked_node_id=event.node_to_edit)
            res = await self.bot.edit_message_text(
                chat_id=event.user.outer_id, text=event.text, message_id=message_id_to_edit, reply_markup=keyboard
            )
        else:
            keyboard = await self.get_keyboard(event)
